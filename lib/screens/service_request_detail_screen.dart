...
const Color kPendingPaymentColor = Color(
  0xFFFF9800,
); // Orange for Awaiting Payment
...
class _ServiceRequestDetailsScreenState
    ...
    if (markCompleted) {
      final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
            "Are you sure you want to mark this work as completed? The status will change to 'Awaiting Payment'.",
          ),
          actions: [
            TextButton(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              markCompleted
                  ? 'Job Completed. Awaiting Payment.'
  ...
  Future<void> _submitInitialAcceptance() async {
    if (_priceController.text.isEmpty || _estimatedEndDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please enter Price and Estimated End Date"),
        ),
      );
      return;
    }
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text(
              "Decline",
              style: TextStyle(color: kDeclineColor),
            ),
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    ...
    final expectedEnd = details?['expected_end'] != null
        ? DateFormat(
            'MMM d, y',
          ).format(DateTime.parse(details!['expected_end']))
        : "Not Set";
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () =>
                              _updateJobStatus(markCompleted: true),
